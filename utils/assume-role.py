...
# Programmatic use:
#
#   lookup_role_arn(roleName)           - Retrieves the ARN corresponding to
#                                         a simple name
#   generate_session_name()             - Generates a UUID-based session ID that
#                                         includes the user's account and username
#                                         if available.
#   assume_role(nameOrArn, optMfaCode)  - Returns credentials for an assumed role,
#                                         specified either by name or ARN, with an
#                                         optional MFA code.
#
################################################################################
...
def assume_role(arnOrName, optMfaCode=None):
    """ Assumes a role and returns its credentials.

        May be passed either a role name (in which case the ARN is retrieved) or an
        ARN (in which case it's used as-is). May also be passed an optional MFA code
        generated by the current user's MFA device.
    """
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        credentials = assume_role(sys.argv[1], sys.argv[2])
    else:
